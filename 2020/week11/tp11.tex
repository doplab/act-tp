...
\begin{itemize}
    ...
    \item \lstinline{attaque:}\text{(int)} représentant une valeur qui sera utilisée pour calculer le nombre de points de dégâts infligés à l'adversaire.
    \item \lstinline{défense:}\text{(int)} représentant une valeur qui sera utilisée pour calculer le nombre de points de dégâts reçus.
\end{itemize}
...
\begin{Exercice}[10 minutes] attack(String type Combattant other)\\
    Définir une méthode \lstinline{attack(String type, Combattant other)} qui consiste à retirer des points de vie au combattant \lstinline{other} en fonction de l'attaque de l'instance appelée, du type d'attaque sélectionné et de la défense de \lstinline{other}. \\
...
\section{Manipulation de graphes en POO}
