...
\begin{conseil}
   ...
   La première fonction sera la fonction de recherche binaire récursive qui prendra en paramètren la liste, l'index du premier élément de la liste,l'index du dernier élément de la liste et e. Cette fonction retournera l'index de l'un des éléments le plus proche de e. 
...
\end{enumerate}
\begin{solution}
    \textbf{Faux}: À l'excéption des fonctions intégrées (il s'agit des fonctions déjà intégrées au language python telles que \lstinline{print(), len(), abs(), etc}... une fonction doit nécessairement être définie \textbf{avant} d'être appelée.
\end{solution}
\end{Exercice}
