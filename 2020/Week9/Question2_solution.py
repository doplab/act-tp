...
def nearest_neighbor(start, point_set):#start correspond au point de départ, point_set correspond à l'ensemble des points
    for i in range(len(point_set)):#on parcourt tous les points de l'ensemble
        ...
        #Cette partie du code détermine si le point actuellement considéré, est plus proche du point de départ que les points parcourus jusqu'ici. Si c'est le cas, on redéfinit la distance minimale et "enregistre" les coordonées du point.
...
#Devrait retourner [2,5]

print("Le voisin le plus proche est ", point, " avec une distance de ", distance)

#Il n'est pas important de comprendre le bout de code suivant, on l'utilise pour visualiser le résultat.
