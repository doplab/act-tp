...
\begin{Exercice}[10 minutes] \textbf{Complexité} \\
        ...
        Soient $n$ et $m$ les tailles respectives de \lstinline{L} et de \lstinline{M}, on obtient une complexité de $O(n) + O(m) = O(\max\{n,m\})$. 
...
\begin{Exercice}[10 minutes] \textbf{Complexité} \\
    ...
    \begin{solution}
    L'algorithme est composée d'une boucle qui va itérer sur $\log(n)$ éléments et 
...
\begin{Exercice}[10 minutes] \textbf{Complexité \optionnel} \\        
    ...
    \begin{solution} 
    L'algorithme est composé de 2 boucles \textbf{imbriquées}. L'algorithme va sommer les indices $i$ et $j$ à chaque itération et les rajouter à $m$.
    Cela veut dire que nous parcourons la liste un maximum de $n \times n$ fois, $n$ étant la taille de la liste. La complexité de l'algorithme est ainsi $O(n^2)$.
    \end{solution}
