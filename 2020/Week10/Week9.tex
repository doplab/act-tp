...
\section{Tuto : Librairie Random}
Dans cette séance d'exercice, vous aurez besoin d'utiliser la librairie random afin de créer des simulations de Monte-Carlo. Vous trouverez ci-dessous les fonctions importantes de la librairie :
\begin{enumerate}
    \item Premièrement, cette librairie n'est pas utilisable sans que l'on l'importe au préalable. Pour cela, ajoutez \textbf{import random} en haut de votre code. Cela permet d'ajouter "la boîte à outil" des nombres aléatoires à votre code.
    
    \item Pour "tirer" un nombre aléatoire, utilisez \textbf{random.random()}. Cela vous donnera un nombre aléatoire contenu entre 0 et 1.
    \item Si vous avez besoin d'un nombre entier aléatoire contenu entre a et b (borne comprise), utilisez \textbf{random.randint(a,b)}.
    \item Parfois, lorsque l'on programme des simulations de Monte-Carlo, il est difficile de savoir si les résultats obtenus sont correctes car les nombres aléatoires générés à chaque exécution du programme seront différent. Cela rend difficile la comparaison entre certaines modification de votre code. Pour palier à cela, vous pouvez utiliser la fonction \textbf{random.seed(n)}, où n est un nombre entier. Ainsi, la séquence de nombre aléatoire généré sera la même à chaque exécution de votre programme.
\end{enumerate}

Vous trouverez un exemple de ces fonctions dans le fichier \textbf{random.py}.
