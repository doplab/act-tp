...
=======
\usepackage{listings}
>>>>>>> daf9845db3a46079d70d5d82c62a0bb63b65e771
\usepackage{array}
...
Les langages qui seront utilisés pour cette séance sont \textbf{Java} et \textbf{Python}. Assurez-vous d’avoir bien installé \textbf{Pycharm} et \textbf{Netbeans}. Si vous rencontrez des difficultés, n’hésitez pas à vous référer au guide suivant: \url{https://moodle.unil.ch/mod/resource/view.php?id=891556}.

Le code utilisé dans les questions est également disponible sur Moodle.

\begin{section}{Typage}
...
Citez trois types de variables couramment utilisés. Comment les déclareriez-vous en \textbf{Python} et en \textbf{Java}?
    \begin{conseil}
        Se référer aux diapositives du cours ou à la documentation officielle de votre langage de programmation préféré.
    \end{conseil}
\end{Exercice}
    \begin{solution}
        Parmi les types de variables les plus couramment utilisées, on peut citer:
        \begin{itemize}
            \item Les entiers (Integer ou int)
            \item Les nombres à virgule flottante (Float)
            \item Les chaînes de caractères (String ou str)
            \item Les booléens (Boolean ou bool)
        \end{itemize}
        Pour le déclarer, on utilise la syntaxe suivante:
        \\
        En \textbf{Java}:
