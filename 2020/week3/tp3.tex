...
\begin{Exercice}[10 minutes] \textbf{Floating point}\\
    ...
    % TODO: Pas sûr d'avoir compris le sens de biais.
...
\begin{Exercice}[5 minutes] \textbf{Détection d'erreurs} \\
    ...
    \end{conseil}
    \begin{solution}
    	Le premier code va soulever une erreur de type \lstinline{Type error}, même si la fonction n'est jamais appelée. En Java, le typage est statique, et de ce fait, toutes les erreurs seront détectées avant exécution du programme.

Le deuxième code en revanche ne va pas soulever d'erreur. En python, le typage est dynamique et de ce fait, l'erreur ne sera détectée qu'au moment ou le programme sera exécuté. Si on change le \lstinline{True} en \lstinline{False}, la fonction provoquant l'erreur va être appelée, et dans ce cas ci, l'erreur sera mise en évidence.\\
    \end{solution}
...
\begin{Exercice}[5 minutes] \textbf{Calculs (multiplication) (Java ou Python)}\\
...
\begin{Exercice}[5 minutes] \textbf{Type (Python uniquement)}\\
...
Les deux exercices suivants consistent à créer de petits jeux bien connus, et relativement rapides à implémenter. \\
