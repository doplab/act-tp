...
Le mot-clé \lstinline{this} en Java est une variable de référence qui fait référence à l'objet actuel. Il est utilisé dans une méthode d'instance ou un constructeur pour accéder aux membres de l'objet actuel, tels que les variables d'instance, les méthodes et les constructeurs. \\
...
\begin{Exercice}[10 minutes]{Héritage - Java}
	...
	En Java, il faut utiliser le mot-clé \lstinline{extends} lorsqu'on définit une classe-fille. Ainsi, l'héritage permet la réutilisation des attributs et méthodes d'une classe existante.

	\begin{enumerate}
		\item Créez une classe-mère \lstinline{Vehicle} ayant un attribut protégé (\lstinline{protected}) de type chaîne de caractères appelé \lstinline{brand}.
		\item Créez un constructeur pour la classe \lstinline{Vehicle} et assignez la valeur passée comme argument à l'attribut \lstinline{brand}.
	...
	\end{solution}
	\begin{solution}
		\lstinputlisting{solutions/Car.java} 
	\end{solution}
\end{Exercice}
...
\begin{itemize}
	...
	\begin{conseil}
		...
		\begin{itemize}
			...
			\item Si aucune des conditions ci-dessus n'est remplie, faire appel à \lstinline{gcd} de façon récursive avec pour premier argument \lstinline{B} et pour deuxième \lstinline{A\%B}.
		\end{itemize}
	\end{conseil}
	...
	\end{conseil}
	\item Dans la méthode \lstinline{equals}, vérifier que l'instance \lstinline{Fraction} (\lstinline{this}) n'est pas \textbf{identique} à l'objet \lstinline{other}. Si les deux objets sont identiques, renvoyer \lstinline{true}. Vérifier également que les deux objets sont de même type. Dans le cas contraire, renvoyer \lstinline{false}. Si les deux objets ont le même type, vérifier qu'ils ont les mêmes attributs. 
	\begin{conseil}
		En Java, pour vérifier que deux objets sont de même type, vous pouvez accéder à la classe de chacun des éléments en utilisant la méthode \lstinline{.getClass()} et les comparer. Une fois que vous avez vérifé le type de l'objet \lstinline{other} passé comme argument, le convertir en \lstinline{Fraction} (utiliser \href{https://moodle.unil.ch/mod/resource/view.php?id=1965120}{type casting de la semaine 3}).
	\end{conseil}
...
\begin{solution}
	...
	L'instruction \lstinline{f1.equals(f2)} fait appel à la méthode \lstinline{equals()} de notre classe \lstinline{Fraction} et exécute les instructions que nous avons définies dans cette méthode.
	L'instruction \lstinline{==} compare les références, alors que \lstinline{.equals()} compare les valeurs.
\end{solution}
...
\begin{Exercice}[15 minutes]{Création d'une classe abstraite}
	...
	\lstinputlisting{code/Animal.java} 
	...
	\begin{itemize}
		...
		\lstinputlisting{code/variables_q4.java} 
		\begin{conseil}
		...
		\begin{conseil}
			Pensez à utiliser \lstinline{count} pour initialiser la valeur d'\lstinline{id}. Ainsi, dans le constructeur, \lstinline{id} sera égal à \lstinline{++count}.
		\end{conseil}
...
\end{itemize}
		\begin{solution}  
			...
			La variable count est un attribut de classe (mot clé static), il est donc partagé part toutes les instances de la classe.
			Dans le constructeur on voit que count est incrémenté à chaque appel du constructeur. La variable représente donc le nombre d'instance de la classe.
			Pour l'attribut id on demande à ce qu'il soit incrémenté à chaque instanciation de la classe, ce que fait déjà count, c'est pour ça que \lstinline{this.id = ++count}.
			Cela garanti d'avoir un id unique pour chaque instance.
		\end{solution}
	\end{Exercice}
	...
	\begin{conseil}
		Un attribut protégé est accessible aussi bien dans la classe-mère que dans la classe-fille. On utilise le mot-clé \lstinline{protected} pour rendre des attributs protégés. \\
	...
	\begin{solution}
		\lstinputlisting[language=java]{solutions/Figure.java} 
	\end{solution}
	\begin{solution}
		\lstinputlisting[language=java]{solutions/Carre.java} 
	\end{solution}
	\begin{solution}
		\lstinputlisting[language=java]{solutions/Rectangle.java}
	\end{solution} 
	\begin{solution}
		\lstinputlisting[language=java]{solutions/FigureMain.java} 
	\end{solution}
	\end{Exercice}
...
\lstinputlisting{code/IMakeSound.java} 
...
\lstinputlisting{code/fin_interface.java} 

\begin{solution}
\lstinputlisting{solutions/Drinkable.java} 
\end{solution}
\begin{solution}
\lstinputlisting{solutions/Edible.java} 
\end{solution}
\begin{solution}
\lstinputlisting{solutions/Food.java} 
\end{solution}
\begin{solution}
\lstinputlisting{solutions/Soup.java} 
\end{solution}
