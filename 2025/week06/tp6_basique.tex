...
\end{itemize}
\begin{solution}
    La réponse correcte est 1010. La boucle for calcule la représentation binaire de l'entier stocké dans la chaîne \texttt{x} (qui est 101) et l'affichage montre la concaténation de la variable \texttt{result} et \texttt{j}. Ici, \texttt{j} est définie hors de la portée de la boucle for, et cela est la raison pour laquelle on peut y accéder aprés la boucle for. À la sortie de la boucle for, la valeur de \texttt{j} est zéro.
\end{solution}
\end{Exercice}

\section{Récursion en Java et Python (10 minutes)}
\begin{Exercice}[10 minutes] \textbf{isPalindrome() — Python et Java}  Un palindrome est une chaîne de caractères qui se lit de la même manière à l'envers. Plus précisément, soit $f$ une fonction qui renvoie \texttt{True} si \texttt{a} (de longueur $n$ caractères) est un palindrome: \\
\begin{equation}
...
\end{align*}
et le case de base d'une chaîne vide: $f(\texttt{""}) =$ \texttt{True}.\\

Ecrivez la méthode Java \textbf{\texttt{isPalindrome(String arg)}}  et la fonction Python \textbf{\texttt{isPalindrome(arg)}} qui renvoient \texttt{True} si \texttt{arg} est une chaîne de caractères.
