...
Le but de cette séance est de se familiariser avec les algorithmes de recherche. Dans cette série d'exercices, nous manipulerons des listes et collections en Java et Python. Nous reviendrons sur la notion de récursivité et découvrirons les arbres de recherche. Au terme de cette séance, l'étudiant sera en mesure d'effectuer des recherches de façon efficiente sur un ensemble de données. \newline 
    ...
    1. À partir des éléments ci-dessous, écrivez une fonction qui cherche \texttt{x} dans la liste \texttt{L}. \newline 
    
    La fonction doit retourner l'index de l'élément correspondant de la liste si \texttt{x} est dans la liste et -1 si \texttt{x} n'est pas dans la liste (avec \texttt{x = 100}).\\
...
\end{conseil}
    ...
    \begin{conseil}
        ...
        \begin{itemize}
            \item Si \texttt{e} n'est pas dans la liste \texttt{L}: une exception de type \lstinline{ValueError} est levée. 
    ...
    \textbf{Python :}
        ...
        \begin{conseil}
            Complétez la fonction \texttt{plus\_proche\_sequentielle} et exécutez le code.\\\\
            ...
            L'algorithme doit calculer la différence entre \texttt{e} et chaque élément de la liste \texttt{L} en gardant toujours la plus petite différence trouvée. À la fin, il retourne l'élément de la liste correspondant à la plus petite différence.  
        \end{conseil}
...
#Attention à inclure les lignes de codes additionnelles présentes dans le fichier "question6.py" sur Moodle
        ...
        Il contient également un code de ``vérification''. Celui-ci va automatiquement tester votre fonction avec différents paramètres et vérifier si les résultats obtenus correspondent au résultats attendus.
        \end{conseil}
