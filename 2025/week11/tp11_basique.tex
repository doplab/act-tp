...
% Dashed lines for children (representing subtrees)
\draw[dashed] (l) -- ++(-0.95,-0.95);
\draw[dashed] (l) -- ++(0.95,-0.95);
\draw[dashed] (r) -- ++(-0.95,-0.95);
\draw[dashed] (r) -- ++(0.95,-0.95);

% Parent of z (representing parent of the subtree)
\draw[dashed] (z) -- ++(0,0.95);

% Background (light blue rectangle)
\fill[red!10, opacity=0.6, rounded corners] ($(z.north east) + (-0.4,-1.2)$) rectangle ($(l.south west) + (-1.4,-1.9)$) ;
\fill[blue!10, opacity=0.6, rounded corners] ($(z.north east) + (3.1,-1.2)$) rectangle ($(l.south west) + (2.1,-1.9)$) ;

% descriptors
% Define corners of red rectangle
\coordinate (redA) at ($(z.north east) + (-0.5,-1.2)$);
\coordinate (redB) at ($(l.south west) + (-1.1,-1.9)$);
