...
% \end{Exercice}
    ...
    À partir de la liste d'éléments \textbf{triés} ci-dessous, écrivez premièrement \textbf{une fonction récursive} puis \textbf{une fonction itérative} qui cherche \texttt{x} dans la liste. La fonction doit retourner \textbf{l'index} de l'élément correspondant de la liste si \texttt{x} est dans la liste et \lstinline{-1} dans le cas contraire.\\
    ...
    \lstinputlisting{resources/question5_2.py}
        ...
        \begin{conseil}
            ...
            \lstinline{low} : L'indice de départ de la partie de la liste à fouiller (0 au départ).\\
            \lstinline{high} : Le dernier indice de la partie de la liste à fouiller (\lstinline{len(L) - 1} au départ).\\
            \lstinline{key} : La valeur recherchée.\\
