# Définition de la fonction ayant pour argument une liste et un nombre
def plus_proche_sequentielle(liste, nb):
    # Initialisation de la variable ( -1 car les différences calculées après seront toujours positives)
    diff = -1
    ...
    for elem in liste:
        if diff == -1 or abs(elem-nb) < diff:
            diff = abs(elem-nb)  # nouvelle diff.
            resultat = elem
