...
\begin{attention}
Cette banque de questions est destinée à vous servir de ressource si vous souhaitez vous entraîner davantage. \textbf{La répartition des thèmes abordés dans cette banque de questions peut ne pas refléter celle des thèmes abordés dans l'examen.} \newline 
...
\mcqlist{10010001, 01110001, 01100001, Aucue des réponses n'est correcte}
\end{Exercice}
...
\textbf{Question 38.A} Supposons qu'on ouvre un terminal Windows dans le dossier qui contient ce fichier python, comment pouvons-nous l'exécuter tout en évitant qu'il nous imprime \texttt{Assurez-vous de passer au moins un argument à votre programme}?
...
\textbf{Question 38.B} Selon votre réponse à la question précédente, notez le résultat de l'exécution. 
...
\textbf{Question 38.C} Quelle remarque est correcte par rapport à  la ligne 5: \texttt{if len(arguments) > 1}? 
\mcqlist{
...
\end{lstlisting}
Que sera-t-il affiché à la fin du bloc \texttt{switch} (avant la ligne 15): System.out.println();
...
Indiquez pour chaque fonction addition par quel type on doit remplacer les ??? pour que le programme fonctionne (ex : addition\_5: float).
...
\mcqlist{
    ...
    Le mot-clé public est obligatoire devant toute function.,
...
Quelle affirmation est vrai à propos du programme java ci-dessous:
\begin{lstlisting}[style=javastyle]
...
{L'erreur de syntaxe est ignorée car elle est à l'intérieur d'un try-catch, et le programme s'exécute mais saute la déclaration de la variable.}
}
\begin{solution}
Le programme ne compile pas à cause de l'erreur de syntaxe, donc il ne s'exécute jamais.
\end{solution}
...
\begin{Exercice}[5 minutes]\textbf{Aurore}
Par quoi faut-il remplacer la ligne \#LINE REMOVED pour que le programme suivant :
\begin{lstlisting}[style=pythonstyle]
...
Sélectionnez les affirmations corrects: 
...
\mcqlist{
  ...
  {Les attributs nb\_seats et nb\_mini sont privés},
  {Les attributs nb\_mini et height sont de même type},
  {L'attribut nb\_mini est un attribut de classe},
...
\begin{Exercice}[5 minutes]\textbf{Maxim}
Par quelle valeur faut-il remplacer \#LINE REMOVED afin que le code exécute correctement?
...
\begin{solution}
Option D: Alice et Bon uniquement. Cathy ne peut pas en prendre (car alors Alice et Bob devraient aussi, cela contredit soit Bob soit Cathy).
...
\begin{Exercice}[5 minutes]\textbf{Léa}
Soit le graph décrit par l'adjency matrix si dessous:
...
\begin{Exercice}[5 minutes]\textbf{Sean}
Soit l'algorithme de construction d'un arbre KD avec $K=3$, et un ensemble de points en 3D sous la forme $(x, y, z)$.
