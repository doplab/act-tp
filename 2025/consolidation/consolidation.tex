...
\begin{tabular}{| C{0.1\textwidth} | C{0.15\textwidth} |} 
    \hline
...
\end{Exercice}
   \begin{conseil}
    \begin{itemize}
        \item Pensez à décomposer la valeur de l’\lstinline{Instruction Register} pour obtenir toutes les informations demandées.
        \item  Utilisez la même convention que celle présentée dans les diapositives du cours (Architecture des ordinateurs - semaine 1).
        \item Les données issues de la décomposition de l’\lstinline{Instruction Register} ne sont pas des valeurs brutes, mais des références. Trouvez les tables concordantes pour y récupérer les valeurs.
    \end{itemize}
...
\begin{Exercice}[10 minutes]\\
    \begin{enumerate}
        ...
        \item Déterminer au complément à deux l'opposé (l'inverse additif) de 0110000$_{(2)}$.
    \end{enumerate}
\begin{conseil}
   % Un conseil
   \begin{itemize}
...
\begin{Exercice}[5 minutes]
    ...
    \lstinputlisting{code/conditions.py}
...
\begin{Exercice}[5 minutes] \textbf{Portée des variables} (Python)\\
    ...
    \lstinputlisting{code/portee_variables.py}
 ...
 \begin{Exercice}[10 minutes]\textbf{Fonctions et Exceptions} (Python)\\
    Écrivez une fonction \lstinline{check_name(name)} qui prend en argument le paramètre \lstinline{name} et qui affiche dans la console ``\textit{Bonjour suivi du nom passé en paramètre}''.
...
\begin{Exercice}[5 minutes]\textbf{Gestion d'exceptions} (Java)\\
 ...
 \begin{Exercice}[5 minutes]\textbf{Gestion d'exceptions} (Java)\\
    Qu'affiche le programme suivant ?
    \lstinputlisting{code/Question14.java}
...
\begin{Exercice}[10 minutes] \textbf{Fonctions et itération}
    ...
    \begin{solution}
        \lstinputlisting{code/overlap.py}
    \end{solution}
\end{Exercice}
...
\begin{conseil}
    \begin{itemize}
        \item Pour la version itérative, parcourez toute la chaîne de caractères et incrémentez un compteur lorsque vous avez une voyelle.
        \item Pour la version récursive, diminuez systématiquement la taille de votre chaîne de caractères. Si l'élément actuel est une voyelle, ajoutez 1, sinon, ajoutez 0.
        \item Aidez vous d'une liste contenant toutes les voyelles et de l'instruction \lstinline{in} en \lstinline{Python} (\lstinline{List.contains()} en \lstinline{Java}).
        \item En \lstinline{Python}, pour accéder à un caractère dans une chaîne de caractères, vous pouvez utiliser l'indexation (ex: \lstinline{chaine[0]} pour accéder au premier caractère de la chaîne de caractères \lstinline{chaine}). En \lstinline{Java}, vous pouvez utiliser la méthode \lstinline{charAt()} de la classe \lstinline{String} (ex: \lstinline{chaine.charAt(0)} pour accéder au premier caractère de la chaîne de caractères \lstinline{chaine}).
    \end{itemize}
\end{conseil}
...
\textbf{Python} \\
    \lstinputlisting{code/iteration_recursion_exercice.py} 

\textbf{Java} \\
    \lstinputlisting{code/Question16.java}
...
\textbf{Java :} \\
    ...
    \lstinputlisting{solutions/IterationRecursionSolution.java}
...
\begin{conseil}
   Ces deux programmes comportent des fonctions récursives, lisez bien le code de haut en bas et lorsque la fonction fait appel à elle-même, revenez au début de la fonction et effectuez de nouveau les instructions avec les nouveaux paramètres. Une feuille de papier pourrait vous être utile!
\end{conseil}

\textbf{\\Programme 1 :} \\

    \lstinputlisting{code/recursion_1_exercice.py} 

\textbf{Programme 2 :} \\

    \lstinputlisting{code/recursion_2_exercice.py}
...
\begin{solution}
    ...
    concat/.style={draw, fill=yellow!20, rounded corners, minimum width=5cm, minimum height=0.7cm},
    basecase/.style={draw, fill=blue!10, rounded corners, minimum width=2cm, minimum height=0.7cm},
    return/.style={draw, fill=green!20, rounded corners, minimum width=3cm, minimum height=0.7cm},
]

% Base case at bottom
\node[basecase] (base) at (0, 0) {recursion\_1("n") $\Rightarrow$ "n"};

% Level 4
\node[concat] (c4) at (0, 1.5) {"o" + recursion\_1("n") + "o"};
\draw[decorate, decoration={brace, amplitude=10pt}, thick] 
    (c4.north west) -- (c4.north east) node[midway, above=12pt] {recursion\_1("on")};

% Level 3
\node[concat] (c3) at (0, 3.5) {"h" + recursion\_1("on") + "h"};
\draw[decorate, decoration={brace, amplitude=10pt}, thick] 
    (c3.north west) -- (c3.north east) node[midway, above=12pt] {recursion\_1("hon")};

% Level 2
\node[concat] (c2) at (0, 5.5) {"t" + recursion\_1("hon") + "t"};
\draw[decorate, decoration={brace, amplitude=10pt}, thick] 
    (c2.north west) -- (c2.north east) node[midway, above=12pt] {recursion\_1("thon")};

% Level 1
\node[concat] (c1) at (0, 7.5) {"y" + recursion\_1("thon") + "y"};
\draw[decorate, decoration={brace, amplitude=10pt}, thick] 
    (c1.north west) -- (c1.north east) node[midway, above=12pt] {recursion\_1("ython")};

% Level 0
\node[concat] (c0) at (0, 9.5) {"P" + recursion\_1("ython") + "P"};
\draw[decorate, decoration={brace, amplitude=10pt}, thick] 
    (c0.north west) -- (c0.north east) node[midway, above=12pt] {recursion\_1("Python")};

% Return values to the right
\node[return, right=1.5cm of c4] (ret4) {"o" + "n" + "o" = "ono"};
\node[return, right=1.5cm of c3] (ret3) {"h" + "ono" + "h" = "honoh"};
\node[return, right=1.5cm of c2] (ret2) {"t" + "honoh" + "t" = "thonoht"};
\node[return, right=1.5cm of c1] (ret1) {"y" + "thonoht" + "y" = "ythonohty"};
\node[return, right=1.5cm of c0] (ret0) {"P" + "ythonohty" + "P" = "PythonohtyP"};

% Draw arrows from concat to returns
\draw[-Stealth, red, dashed, thick] (c4) -- (ret4);
\draw[-Stealth, red, dashed, thick] (c3) -- (ret3);
\draw[-Stealth, red, dashed, thick] (c2) -- (ret2);
\draw[-Stealth, red, dashed, thick] (c1) -- (ret1);
\draw[-Stealth, red, dashed, thick] (c0) -- (ret0);

% % Draw vertical arrows showing flow
% \draw[-Stealth, thick, blue] (base) -- (c4);
% \draw[-Stealth, thick, blue] (c4) -- (c3);
% \draw[-Stealth, thick, blue] (c3) -- (c2);
% \draw[-Stealth, thick, blue] (c2) -- (c1);
% \draw[-Stealth, thick, blue] (c1) -- (c0);

% Add title
\node[above=1.5cm of c0, xshift=3cm, draw=none, fill=none, font=\large\bfseries] {Diagramme de récursivité (Recursion Tree): recursion\_1(\textquotedbl Python\textquotedbl)};

% % Add legend
% \node[below=1cm of base, draw=none, fill=none, align=left, font=\footnotesize] {
%     \textbf{Legend:}\\
%     \tikz{\node[concat,minimum width=1.5cm,minimum height=0.5cm] {Concatenation};} \\
%     \tikz{\node[return,minimum width=1.5cm,minimum height=0.5cm] {Return Value};} \\
%     \tikz{\node[basecase,minimum width=1.5cm,minimum height=0.5cm] {Base Case};}
% };
...
\resizebox{0.93\textwidth}{!}{%
    ...
    call/.style={draw, fill=blue!10, rounded corners, minimum width=2.5cm, minimum height=0.6cm},
    print/.style={draw, fill=orange!20, rounded corners, minimum width=1.8cm, minimum height=0.5cm},
    basecase/.style={draw, fill=green!20, rounded corners, minimum width=2cm, minimum height=0.6cm},
    edge/.style={draw, -Stealth, thick, blue!70}
]

% Level 0 - Root
\node[call] (root) at (0, 0) {recursion\_2(\shortstack{["J", "adore","Python"]})};

% Level 1 - First call
\node[call] (L1) at (-5.5, -3) {recursion\_2(\shortstack{["adore","Python"]})};

% Level 2 - Left base case (no print here)
\node[basecase] (L2a) at (-9, -5) {recursion\_2(["Python"])};

% NEW print under L2a
\node[print] (L2a_print) at (-9, -7) {print("Python")};

% Level 2 - Print between calls
\node[print] (L2print) at (-5.5, -7) {print("adore")};

% Level 2 - Right branch base case (no print here)
\node[basecase] (L2b) at (-2.5, -5) {recursion\_2(["Python"])};

% NEW print under L2b
\node[print] (L2b_print) at (-2.5, -7) {print("Python")};

% Level 1 - Print at root level
\node[print,  align=left] (L1print) at (0, -7) {print("J")};

% Level 1 - Second call (mirror of first)
\node[call] (R1) at (5.5, -3) {recursion\_2(\shortstack{["adore","Python"]})};

% Level 2 - Right side, left base case (no print here)
\node[basecase] (R2a) at (2.5, -5) {recursion\_2(["Python"])};

% NEW print under R2a
\node[print] (R2a_print) at (2.5, -7) {print("Python")};

% Level 2 - Right side, print between calls
\node[print] (R2print) at (5.5, -7) {print("adore")};

% Level 2 - Right side, right base case (no print here)
\node[basecase] (R2b) at (9, -5) {recursion\_2(["Python"])};

% NEW print under R2b
\node[print] (R2b_print) at (9, -7) {print("Python")};

% Draw edges
\draw[edge] (root) -- (L1);
\draw[edge] (L1) -- (L2a);
\draw[edge] (L2a) -- (L2a_print);
\draw[edge] (L1) -- (L2print);
\draw[edge] (L1) -- (L2b);
\draw[edge] (L2b) -- (L2b_print);
\draw[edge] (root) -- (L1print);
\draw[edge] (root) -- (R1);
\draw[edge] (R1) -- (R2a);
\draw[edge] (R2a) -- (R2a_print);
\draw[edge] (R1) -- (R2print);
\draw[edge] (R1) -- (R2b);
\draw[edge] (R2b) -- (R2b_print);

% Add curly brace
\draw[decorate, decoration={brace, amplitude=8pt}, thick] 
    (root.north west) -- (root.north east) node[midway, above=10pt] {recursion\_2(["J", "adore", "Python"])};

% Add title
\node[above=0.8cm of root, draw=none, fill=none, font=\large\bfseries]
 {Diagramme de récursivité (Recursion Tree): recursion\_2(["J", "adore", "Python"])};

% Execution order numbers
% ADD THE NODES WITH NUMBERS HERE

% Output summary box
\node[below=3cm of R2b, draw, xshift=-9cm, fill=yellow!10, rounded corners,
      minimum width=8cm, align=left, font=\small] {
    \textbf{Ordre d'affichage:}\\
     Python →  adore →  Python →  J →  Python →  adore →  Python
};
...
\begin{Exercice}[15 minutes] \textbf{Itération et Récursivité}:\\
    ...
    \begin{conseil}
        Utilisez l'argument \lstinline{end} de la fonction \lstinline{print} de Python pour ne pas faire de passage à la ligne pour chaque caractère et utilisez les caractères “-” et “$|$” pour générer les lignes et les colonnes, respectivement.
    \end{conseil}
...
\begin{Exercice}[15 minutes]\textbf{Jeu de rôle}
    ...
    \begin{itemize}
       ...
       personnage. Cette méthode nous permettra d'afficher dans la console les informations du personnage lorsqu'on affiche dans la console une instance de \lstinline{Personnage}.
        ...
        \lstinputlisting[language=java]{code/RolePlayMere.java}
        ...
        \lstinputlisting[language=java]{code/RolePlayFilles.java}
    \end{itemize}
    
    \begin{solution}
        \lstinputlisting[language=java,lastline=50]{solutions/Personnage.java}
    \end{solution}
    \begin{solution}
        \lstinputlisting[language=java,firstline=51, firstnumber=51]{solutions/Personnage.java}
    \end{solution}
    
    \begin{solution}
        \lstinputlisting[language=java,lastline=54]{solutions/Guerrier.java}
    \end{solution}
    \begin{solution}
        \lstinputlisting[language=java,firstline=55, firstnumber=55]{solutions/Guerrier.java}
    \end{solution}
    \end{Exercice}
...
En Python, \texttt{\textquotesingle Hello\textquotesingle} équivaut à \texttt{"Hello"}. 
...
\begin{solution}
    ...
    \lstinputlisting{code/double_nombre.py}
...
\begin{Exercice}[2 minutes]\\
Lorsqu'on fait appel à une fonction, les arguments doivent nécessairement avoir le(s) même(s) noms tel(s) que définis dans la fonction. Exemple:\\
\begin{lstlisting}
...
def nombre_decimal_pi(arg):
    if arg > 15:
        arg = 15
    elif arg < 0:
        arg = 1
    resultat = round(math.pi,arg) 
    return resultat
