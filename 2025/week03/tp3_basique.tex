...
\begin{Exercice}[5 minutes] 
    Écrire le code Java qui demande à l'utilisateurice d'entrer l'étape d'éxecution d'une instruction et affiche les étapes qui reste à exécuter (y compris l'étape courante). 
    Si l'utilisateurice entre une étape invalide le code affiche \texttt{L'étape n'est pas valide.}
    ...
    la saisie texte de l'utilisateurice, utilisez \texttt{nextLine()} telle qu'elle est définie dans \href{https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/util/Scanner.html\#nextLine()}{la documentation Java en ligne}.
    \end{conseil}


   \begin{Example}{Exemple}
Si l'utilisateurice entre \texttt{decode}, le programme va afficher:
...
\vspace{0pt}
Vous êtes donné l'arborescence à gauche d'un système de fichiers (filesystem).
...
\textbf{Re-faire} cet exercice en utilisant \texttt{input} pour lire la saisie de l'utilisateurice.
\begin{conseil}
