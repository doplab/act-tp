...
\begin{Exercice}[10 minutes] \textbf{Somme des chiffres} \\
    ...
    \begin{conseil}
    \begin{itemize}
        \item Pour obtenir les chiffres qui composent un nombre, utilisez l'opérateur \lstinline{\%} (modulo - \url{https://fr.wikipedia.org/wiki/Modulo\_(op\%C3\%A9ration)}).
        \item Pour obtenir le nombre 12 à partir du nombre 126, il vous suffit de faire la division entière par 10. En Python, on utilise l'opérateur \lstinline{\/\/}. Exemple : 126 \lstinline{\/\/} 10 = 12.
        \item En Java, la division entre deux variables de type \lstinline{int} est entière, et vous n'aurez ainsi qu'à utiliser l'opérateur de division normal \lstinline{\/}. Exemple: 126 \lstinline{\/} 10 = 12.
    \end{itemize}
    ...
    \begin{solution}
    \textbf{Python :}
        \lstinputlisting{solutions/SumDigits.py}
                
    \textbf{Java :}
       \lstinputlisting{solutions/SumDigits.java}
...
\section{Complexité (40 minutes)}
 ...
 \begin{figure}[H]
        \centering
        \includegraphics[width=9cm]{ressources/Complexity.png}
...
\begin{Exercice}[10 minutes] \textbf{Complexité} \\
    ...
    Quelle est la complexité des programmes ci-dessous?\\
    \textbf{Python :}
...
public static int algo(int n) {
    ...
    \begin{solution}
        L'algorithme est composé d'une boucle qui incrémente une variable \lstinline{s}. Il effectue l'opération d'addition $10*n$ fois et par conséquent a une complexité de $O(n)$.
    \end{solution}
\end{Exercice}


\begin{Exercice}[10 minutes] \textbf{Complexité} \\
    Quelle est la complexité des programmes ci-dessous ($n$ étant la longueur de la liste \texttt{L})?\\
    \textbf{Python :}
    \begin{lstlisting}[language=Python]
...
public static int algo(int[] L, int M) {
    ...
    \begin{solution}
    L'algorithme est composé d'une boucle \lstinline{while} qui va parcourir une liste \lstinline{L} jusqu'à trouver une valeur dans la liste qui est supérieure à \lstinline{M}. Ainsi, dans le pire des cas, l'algorithme parcourt toute la liste, et a donc une complexité de $O(n)$, $n$ étant la taille de la liste.
    \end{solution}
\end{Exercice}
\begin{Exercice}[10 minutes] \textbf{Complexité} \\
    Quelle est la complexité des programmes ci-dessous?\\
...
public static void algo(int[] L, int[] M) {
    ...
    for (int j=0; j < m; j++){
        ...
        Comme on effectue une boucle et après l'autre les complexités s'additionnent.
        \end{solution}
\end{Exercice}
...
public static int algo(int n) {
    ...
    Cela veut dire que nous parcourons la liste $\frac{n^{2}}{2}$ $- \frac{n}{2}$, $n$ étant la taille de la liste. Comme vu dans les conseils avant, les constantes et les termes dominés sont ignorés. La complexité de l'algorithme est ainsi de $O(n^2)$.
    \end{solution}
...
\begin{Exercice}[15 minutes] \textbf{Insertion dans une liste triée - Python}\\
    ...
    \lstinputlisting{ressources/IntInsertion.py}

    \begin{Example}{\faTerminal \quad Exemple}
        En passant les arguments suivants à votre programme: \texttt{n=5} et \texttt{l=[2,4,6]}. Ce dernier devra retourner \lstinline{l =[2,4,5,6]}
    \end{Example}

    \begin{solution}
        \lstinputlisting{solutions/IntInsertion.py}
    \end{solution}
...
\begin{Exercice} [30 minutes] \textbf{Tri fusion (Merge Sort) - Python} \\
    ...
    \begin{itemize}
        \item \textbf{Python :}
            \lstinputlisting{ressources/MergeSort.py}
    \end{itemize}
    ...
    \begin{conseil}
\textbf{Théorème 1} \quad La récurrence
...
où $a$, $b$, $c$ et $k$ sont des constantes, se résout comme suit :

\begin{align*}
T(n) &\in O(n^{k}) \quad \text{si } a < b^{k} \\
T(n) &\in O(n^{k} \log n) \quad \text{si } a = b^{k} \\
T(n) &\in O(n^{\log_{b}{a}}) \quad \text{si } a > b^{k}
\end{align*}
    ...
    \begin{solution}
        \textbf{Python :}
        \lstinputlisting[language=python, lastline=30]{solutions/MergeSort.py}
    \end{solution}

    \begin{solution}
\lstinputlisting[language=python, firstline=31, firstnumber=31]{solutions/MergeSort.py}
    \end{solution}
...
\begin{Exercice} [20 minutes] \textbf{Tri à bulles (Bubble Sort) - Python \optionnel} \\
    ...
    \begin{itemize}
            \item \textbf{Python :}
                \lstinputlisting{ressources/BubbleSort.py} 
        \end{itemize}
        ...
        \begin{solution}
        \textbf{Python :}
        \lstinputlisting{solutions/BubbleSort.py}
    \end{solution}
    \begin{solution}
