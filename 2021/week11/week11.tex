...
\newpage
\section{Treap}
\begin{Exercice}[20 minutes]\textbf{Insertion dans un Treap: Python} \optionnel\\
    Un Treap est un arbre binaire où chaque sommet $v$ a 2 valeurs, une clé \lstinline{v.key} et une priorité \lstinline{v.priority}. Un treap est une
    combinaison d'un arbre de recherche binaire et d'une heap. Ainsi, le treap a la même structure qu'un arbre de recherche binaire dont les nœuds sont
    insérés par ordre de priorité. \\

    Dans cet exercice, vous allez implémenter une fonction pour insérer un nœud dans un treap. Vous avez le squelette de code suivant à remplir.
    ...
    \begin{solution}
    \lstinputlisting[language = python]{solutions/treap_solution_1.py}
    \end{solution}
    \begin{solution}
        \lstinputlisting[language = python]{solutions/treap_solution_2.py}
    \end{solution}
