...
Les langages qui seront utilisés pour cette séance sont Java et Python. Assurez-vous d'avoir bien installé Intellij. Si vous rencontrez des difficultés, n'hésitez pas à vous référer au guide suivant: \href{https://moodle.unil.ch/mod/resource/view.php?id=2218868}{tutoriel d'installation des outils (MacOS) et prise en main de l'environnement de travail} ou \href{https://moodle.unil.ch/mod/resource/view.php?id=2218867}{tutoriel d'installation des outils (Windows)}.

\section{Input / Output}

\begin{Exercice}[5 minutes] \textbf{Output (Java ou Python)}\\
   Créez une variable \textit{nom} de type chaîne de caractères contenant votre nom, et une autre \textit{prenom} (str) contenant votre prénom puis affichez : "Bonjour, \textit{prenom nom}". \\
...
\begin{Exercice}[5 minutes] \textbf{Input (Java ou Python)}\\
   En vous référant à l'exercice précédent (\textbf{Output (Java ou Python)}), demandez à l'utilisateur d'entrer son nom et son prénom au lieu d'initialiser vous-même les variables. En Java, utilisez la classe \lstinline{Scanner} et la méthode \lstinline{nextLine()}, et en Python, utilisez la fonction \lstinline{input()}. \\
...
\begin{Exercice}[5 minutes] \textbf{Complément à 1}\\
    ...
    \begin{conseil}
        \begin{itemize}
        	\item Pour inverser les bits, remplacer chaque 0 par 1 et chaque 1 par 0.
...
\begin{Exercice}[10 minutes] \textbf{Floating point}\\
    ...
    \begin{itemize}
    	...
    	\item L'exposant et la mantisse sont des entiers positifs non signés.
    ...
    \begin{solution}
        \begin{itemize}
        	...
        	\item 1 + mantisse = 1 + 01000001000000000000001$_{(2)}$ = 1 + 1*$2^{-2}$ + 1*$2^{-8}$ + 1*$2^{-23}$ $\approx$ 1.254$_{(10)}$
        	\item Valeur = Signe * Mantisse * $2^{exposant-127}$ $\approx$ 1 * 1.254 * $2^{54}$ = 2.259 * $10^{16}$
        \end{itemize}
    \end{solution}
\end{Exercice}
...
\begin{Exercice}[5 minutes]  \textbf{Les types de variables}\\
    ...
    \begin{solution}
       ...
       Les principaux types de variables sont les suivants, avec leur nom en Java indiqué entre parenthèses : \lstinline{int (int)}, \lstinline{float (float)}, \lstinline{str (String)}, \lstinline{bool (boolean)}.\\
...
\begin{Exercice}[10 minutes]  \textbf{Typage statique et dynamique}\\
    ...
    \begin{conseil}
        ...
        En Java, le typage est statique: lorsqu'une variable est déclarée, un type lui est associé. Ce type peut être explicitement indiqué par le programmeur ou, dans certains cas, déduit automatiquement par Java lors de la compilation. \newline
        
        Une fois le type de la variable déterminé, il ne peut plus être modifié. La variable ne peut donc recevoir que des valeurs compatibles avec ce type. Si vous tentez de lui attribuer une valeur d'un type incompatible, Java signalera une erreur de compilation. \\
    ...
    \begin{solution}
        ...
        \begin{itemize}
        	\item Programme 1: OK
	\item Programme 2 : Ce code ne fonctionnera pas car on essaie de changer le type de la \texttt{variableTwo}, ce qui est impossible avec un typage statique comme en java. Ici le type de la variable est déduit lors de la compilation du programme.
	\item Programme 3: Tout comme le programme précédent, ce code ne fonctionnera pas car on essaye de changer le type de la \texttt{variableTwo}, ce qui est impossible avec un typage statique comme en java.
	\item Programme 4: Ce code ne fonctionnera pas car on essaie de changer le type de la \texttt{variableOne}, ce qui est impossible avec un typage statique comme en java. Ici le type de la variable est déduit lors de la compilation du programme.
        	\item Programme 5: OK
	\item Programme 6: Ce code ne fonctionnera pas car on essaie de changer le type de la \texttt{variableOne}, ce qui est impossible avec un typage statique comme en java. \\
        \end{itemize}
...
\section{Opérateurs et conditions Booléennes}
Le principe d'une valeur booléenne est qu'elle ne puisse contenir que 2 valeurs possibles, soit \lstinline{True} (\lstinline{true} en Java), soit \lstinline{False} (\lstinline{false} en Java). Il est possible de les définir en leur associant une
de ces valeurs d'emblée ou de les obtenir en effectuant une opération booléenne comme par exemple une comparaison. Pour ce faire, il faut utiliser des opérateurs booléens.
Voici les plus utilisés : est égal (\lstinline{==} en Python et en Java), n'est pas égal (\lstinline{!=} en Python et en Java), est strictement plus petit (\lstinline{<} en Python et en Java), est plus petit ou égal (\lstinline{<=} en Python et en Java), est strictement plus grand (\lstinline{>} en Python et en Java), est plus grand ou égal (\lstinline{>=} en Python et en Java).
Si la condition est satisfaite, on obtiendra \lstinline{True}, si elle ne l'est pas, on obtiendra \lstinline{False}.
L'utilisation de l'opérateur de négation (\lstinline{not} en Python et \lstinline{!} en Java) inversera le résultat.\\
...
Il existe toutefois une différence importante entre les deux langages : 
avec la syntaxe classique de \texttt{switch} en Java utilisant les deux-points, 
l'exécution peut continuer dans les \texttt{case} suivants tant qu'aucune 
instruction \texttt{break} n'est rencontrée. Ce comportement est appelé 
\textit{fall-through}. En Python, \texttt{match} n'a pas ce comportement : 
seul le premier \texttt{case} correspondant est exécuté.
...
\begin{conseil}
    ...
    \end{itemize}
\end{conseil}
\item Afficher la longueur de la chaîne de caractères en Python et en Java.
\begin{conseil}
