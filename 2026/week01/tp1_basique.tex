...
\ShowConseiltrue
\titre
\cours{Architecture des ordinateurs - Exercices de base}
...
\begin{Exercice}[5 minutes]  \textbf{Conversion $Base_{10}$ - $Base_2$}\\
    ...
    \begin{conseil}
        ...
        \begin{tabular}{| C{0.15\textwidth} | *{8}{C{0.065\textwidth}|}}
            \hline
            \textbf{puissance} & $2^{7}$ & $2^{6}$ & $2^{5}$ & $2^{4}$ & $2^{3}$ & $2^{2}$ & $2^{1}$ & $2^{0}$ \\ [0.5ex] 
            \hline
            \textbf{valeur} & 128 & 64 & 32 & 16 & 8 & 4 & 2 & 1 \\ [0.5ex] 
            \hline
        \end{tabular} \\
    ...
    \textbf{Méthode 1 pour 10 :}
        \medskip
        \begin{tabular}{| C{0.19\textwidth} | C{0.55\textwidth} |} 
        \hline
        \textbf{nombre} & \textbf{reste (après division par 2)}\\ [0.5ex]
        \hline
        10 &  0 (1er bit de droite à gauche) \\ [0.5ex] 
        \hline
        5 & 1 \\ [0.5ex] 
        \hline
        2 & 0 \\ [0.5ex] 
...
Ici, 45 - 32 = 13, puis 13- 8 = 5, puis 5-4 = 1, puis 1-1 = 0 \\
    ...
    \begin{tabular}{| C{0.19\textwidth} | C{0.55\textwidth} |} 
        ...
        \textbf{nombre} & \textbf{reste (après division par 2)}\\ [0.5ex]
        \hline
        45 &  1 (1er bit de droite à gauche) \\ [0.5ex] 
        \hline
        22 & 0 \\ [0.5ex] 
        \hline
        11 & 1 \\ [0.5ex] 
...
\begin{tikzpicture}[scale=1.3]
    ...
    \node[left] at (0,3.5) {\Huge 45};
    \node[left] at (0,2.5) {\Huge -44};
    ...
    \node[left] at (1,2.5) {\Huge 22};
    \draw[thick] (1,3) -- (1,2);        % vertical down
    \node[left] at (1,1.5) {\Huge -22};
    ...
    \node[left] at (2,1.5) {\Huge 11};
    \draw[thick] (2,2) -- (2,1);        % vertical down
    \node[left] at (2,0.5) {\Huge -10};
    \node[right, draw=blue, thick, inner sep=2pt] at (2.2-\x,0.5-\y) {\Large R:1};
    \draw[thick] (2,1) -- (3,1);        % horizontal right
    \node[above] at (2.5,1) {\Huge 2};
    \node[left] at (3,0.5) {\Huge 5};
    \draw[thick] (3,1) -- (3,0);        % vertical down
    \node[left] at (3,-0.5) {\Huge -4};
    \node[right, draw=blue, thick, inner sep=2pt] at (3.2-\x,-0.5-\y) {\Large R:1};
    \draw[thick] (3,0) -- (4,0);        % horizontal right
    \node[above] at (3.5,0) {\Huge 2};
    \node[left] at (4,-0.5) {\Huge 2};
    \draw[thick] (4,0) -- (4,-1);       % vertical down
    \node[left] at (4,-1.5) {\Huge -2};
    \node[right, draw=blue, thick, inner sep=2pt] at (4.2-\x,-1.5-\y) {\Large R:0};
    \draw[thick] (4,-1) -- (5,-1);      % horizontal right
    \node[above] at (4.5,-1) {\Huge 2};
    \node[left] at (5,-1.5) {\Huge 1};
    \draw[thick] (5,-1) -- (5,-2);      % vertical down
    \node[left] at (5,-2.5) {\Huge -0};
...
\vspace*{0.5cm}
    ...
    \end{solution}
    \begin{solution}
    \textbf{Méthode 1 pour 173 :}
    \medskip
        \begin{tabular}{| C{0.19\textwidth} | C{0.50\textwidth} |} 
        \hline
        \textbf{nombre} & \textbf{reste (après division par 2)}\\ [0.5ex]
        \hline
        173 &  1 (1er bit de droite à gauche) \\ [0.5ex] 
        \hline
        86 & 0 \\ [0.5ex] 
        \hline
        43 & 1 \\ [0.5ex] 
...
\begin{Exercice}[15 minutes]  \textbf{Conversion $Base_{10}$ - $Base_3$, $Base_8$, $Base_{16}$}\\
    ...
    \end{conseil}
    \begin{solution}
        Dans cette solution, la méthode 1 sera utilisée pour 52 en base 3, et la méthode 1  pour 254 en base 16. \\
        
        \textbf{Méthode 2 pour 52 (ici, nous sommes en base 3 donc on divise par 3) :} \\
    ...
    \begin{solution}
        ...
        Ici, il faut prendre en compte le nombre de fois qu'on peut multiplier le nombre par la valeur de la puissance. Ici on a 256 qui est trop grand, il faut donc partir sur 16. On voit qu'on peut aller jusqu'à 15*16 qui vaut 240. Donc on entre 15 sous la valeur 16 et on soustrait. 254-240 = 14. Il nous reste donc 14*1, on met alors 14 sous la valeur 1. \\
   		...
   		\begin{tabular}{| C{0.15\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} |} 
            \hline
        ...
        \medskip
        \textbf{Méthode 1 pour 40 (en base 8 donc on divise par 8)}
        \medskip
        \begin{tabular}{| C{0.1\textwidth} | C{0.1\textwidth} |} 
        \hline
        \textbf{nombre} & \textbf{reste}\\ [0.5ex]
...
\begin{Exercice}[15 minutes] \textbf{Conversion $Base_{2}$, $Base_{5}$, $Base_{16}$ en $Base_{10}$}
    \begin{enumerate}
    ...
    \begin{solution}
        ...
        \begin{tabular}{| C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} |} 
            \hline
            \textbf{puissance} & $2^{4}$ & $2^{3}$ & $2^{2}$ & $2^{1}$ & $2^{0}$ \\ [0.5ex] 
            \hline
            \textbf{valeur} & 16 & 8 & 4 & 2 & 1 \\ [0.5ex] 
            \hline
        \end{tabular} \\
        
        Ici on obtient donc 1*16 + 0*8 + 1*4 + 1*2 + 0*1 = $22_{10}$ \\
        ...
        \begin{tabular}{| C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} | C{0.1\textwidth} |} 
            \hline
            \textbf{puissance} & $5^{3}$ & $5^{2}$ & $5^{1}$ & $5^{0}$ \\ [0.5ex] 
            \hline
            \textbf{valeur} & 125 & 25 & 5 & 1 \\ [0.5ex] 
            \hline
        \end{tabular} \\
        
        Ici on obtient donc 4*125 + 3*25 + 2*5 + 1*1 = $586_{10}$ \\
...
\end{tabular}
    ...
    \begin{tabular}{| p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} |} 
        \hline
    retenue & 0 & 0 & 0 & 0 & 0 & 0 & 0 & 0 \\ [0.5ex]
    \hline
        a & 0 & 1 & 0 & 1 & 0 & 1 & 0 & 1 \\ [0.5ex] 
        \hline
        b & 1 & 0 & 1 & 0 & 1 & 0 & 1 & 0 \\ [0.5ex] 
        \hline
        a + b & 1 & 1 & 1 & 1 & 1 & 1 & 1 & 1 \\ [0.5ex]
        \hline
    \end{tabular}\\ \\
    On obtient donc 11111111$_{(2)}$\\
        ...
        \begin{tabular}{| p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} |} 
            \hline
	    retenue & 1 & 1 & 1 & 0 & 0 & 0 & 0 & 0 \\ [0.5ex]
	    \hline
            a & 0 & 1 & 1 & 1 & 0 & 1 & 0 & 0 \\ [0.5ex] 
            \hline
            b & 0 & 0 & 0 & 1 & 1 & 0 & 1 & 0 \\ [0.5ex] 
            \hline
            a + b & 1 & 0 & 0 & 0 & 1 & 1 & 1 & 0 \\ [0.5ex]
            \hline
        \end{tabular} \\

        On obtient donc 10001110$_{(2)}$ \\
        \begin{enumerate}
...
retenue & 0 & 0 & 0 & 0 & 0 & 0 & 0 & 0 \\ [0.5ex]
\hline
          a & 0 & 1 & 1 & 1 & 1 & 1 & 1 & 1 \\ [0.5ex] 
    \hline
          b & 0 & 1 & 0 & 0 & 0 & 0 & 0 & 0 \\ [0.5ex] 
    \hline
    a - b   & 0 & 0 & 1 & 1 & 1 & 1 & 1 & 1 \\ [0.5ex]
    \hline
\end{tabular} \\
On obtient donc 00111111$_{(2)}$ \\


Pour la deuxième question : \\
\begin{tabular}{| p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} |} 
    \hline
retenue & 1 & 1 & 1 & 1 & 1 & 1 & 1 & 0 \\ [0.5ex]
\hline
    ...
    \hline
\end{tabular} \\
On obtient donc 01111111$_{(2)}$ \\

Pour la troisième question : \\
\begin{tabular}{| p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} | p{1cm} |} 
    \hline
retenue & 1 & 0 & 1 & 0 & 1 & 0 & 1 & 0 \\ [0.5ex]
\hline
    a & 1 & 0 & 1 & 0 & 1 & 0 & 1 & 0 \\ [0.5ex] 
    \hline
    b & 0 & 1 & 0 & 1 & 0 & 1 & 0 & 1 \\ [0.5ex] 
    \hline
    a - b & 0 & 1 & 0 & 1 & 0 & 1 & 0 & 1 \\ [0.5ex]
    \hline
\end{tabular} \\
On obtient donc 1010101$_{(2)}$ \\
 		
 		\begin{enumerate}
        \item 01111111$_{(2)}$ - 01000000$_{(2)}$ = 00111111$_{(2)}$
        \item 10000000$_{(2)}$ - 00000001$_{(2)}$ = 01111111$_{(2)}$
        \item 10101010$_{(2)}$ - 01010101$_{(2)}$ = 01010101$_{(2)}$
    \end{enumerate}
    \end{solution}
...
\end{Exercice}
\newpage
\section{Modèle d'architecture de Von Neumann}
Dans cette section, nous allons simuler une opération d'addition dans le \textbf{modèle d'architecture de Von Neumann}, il va vous être demandé à chaque étape (FDES) de donner la valeur des registres.\\
...
A l'origine, notre \lstinline{Program Counter (PC)} vaut $\texttt{00100000}_{(2)}$ \\ %\lstinline{$00100000_{(2)}$}.

Dans la mémoire, les instructions sont les suivantes:

\begin{tabular}{| C{0.13\textwidth} | C{0.13\textwidth} |} 
    \hline
...
\begin{Exercice}[10 minutes]\textbf{Fetch}\\
...
\begin{Exercice}[10 minutes] \textbf{Decode}
    \begin{enumerate}
        \item Quelle est le type de l'opération à exécuter?
        \item Quelle est le numéro du registre dans lequel le résultat doit être enregistré?
    ...
    \begin{solution}
        ...
        \begin{itemize}
            ...
            \item \lstinline{11}, correspond à \textbf{l'identifiant} du registre où sera sauvegardé le résultat,
            \item \lstinline{01}, correspond à \textbf{l'identifiant} du registre qui stocke le premier nombre,
            \item \lstinline{10}, correspond à \textbf{l'identifiant} du registre qui stocke le deuxième nombre.\\
        \end{itemize}
...
\begin{Exercice}[10 minutes] \textbf{Execute}\\
    Quel est le résultat de l'opération?
    \begin{conseil}
        Toutes les informations permettant d'effectuer l'opération se trouvent dans les données de l'\lstinline{Instruction Register} et la table des registres.
    \end{conseil}
    \begin{solution}

    \begin{tabular}{| C{0.13\textwidth} | C{0.13\textwidth} |} 
        \hline
        Registre 01 & 01101100\\ [0.5ex] 
        \hline
        Registre 10 & 00100101\\ [0.5ex] 
        \hline
        Résultat & 10010001\\ [0.5ex] 
        \hline
    ...
    \medskip
    À la fin de l'étape \textbf{Store} les registres auront les valeurs suivantes : 
    \medskip
    \begin{tabular}{| C{0.13\textwidth} | C{0.13\textwidth} | C{0.13\textwidth}|} 
    \hline
    \textbf{Registre} & \textbf{Valeur} & \textbf{Commentaire} \\ [1ex]
    \hline
    $00$ & $11100011$ & Inchangé\\ [0.5ex] 
    \hline
    $01$ & $01101100$ & Inchangé\\ [0.5ex] 
    \hline
    $10$ & $00100101$ & Inchangé\\ [0.5ex] 
    \hline
    $11$ & $10010001$ & Changé\\ [0.5ex]
    \hline
    \end{tabular}
    \end{solution}
\end{Exercice}
